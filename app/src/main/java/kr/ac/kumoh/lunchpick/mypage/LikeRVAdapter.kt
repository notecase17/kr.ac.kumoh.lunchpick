...
class LikeRVAdapter (
    val context: Context,
    private val like: ItemViewModel,
) :
    RecyclerView.Adapter<LikeRVAdapter.ViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): LikeRVAdapter.ViewHolder {
        val v = LayoutInflater.from(parent.context)
            .inflate(R.layout.taste_rv_item, parent, false)
        return ViewHolder(v)
    }

    override fun onBindViewHolder(holder: LikeRVAdapter.ViewHolder, position: Int) {
        holder.bindItems(like.getLike(position))
    }

    override fun getItemCount(): Int {
        return like.getLikeSize()
    }
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItems(item: ItemModel) {
            ...
            imageViewArea.setImageUrl(
                "${url}images/${item.image}", like.mImageLoader
            )
            name.text = item.name
